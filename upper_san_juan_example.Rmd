...
kable_sj <- kable(head(kable_usj_sites, 10)) %>% 
  ...
  pack_rows("Upper San Juan", 1, 10, label_row_css = "background-color: #666; color: #fff;") %>% 
  scroll_box(width = "600px", height = "300px") %>% 
  footnote(general = "Here is an example of the type of dataframe we can pull from USGS and then either use for analysis/graphing/mapping in R, or export into an Excel document. We could also put together summary statistics in a talbe like this for streamflow, snowpack, etc., perhaps to accompany a graph or map.")
...
meta_snotel_df <- as.data.frame(meta_snotel[['SNTL']]) %>% 
  ...
  rename(State = state, Site = site_name, Start_Date = start, End_Date = enddate, Latitude = latitude, Longitude = longitude, Elevation_ft = elev_ft, County = county, HUC = huc, Site_ID = site_id) %>% 
...
mutate(year_month = yearmonth(date)) %>%  
  mutate(month = month(date)) %>% 
  group_by(year_month) %>% 
  mutate(total_month_swe = sum(Snow_Water_Equivalent)) %>% 
  mutate(total_month_swe = round(total_month_swe, 0)) %>% 
  distinct(year_month, .keep_all = TRUE)
...
# Coerce to a tsibble:
sjp_swe_auto <- as_tsibble(sjp_snotel_clean, index = year_month) %>% 
  tsibble::fill_gaps()

sjp_swe_seasonal <- sjp_swe_auto %>% 
  feasts::gg_season(total_month_swe) +
  scale_y_continuous(expand = c(0,0)) +
  scale_x_date(date_breaks = "1 month",
               date_labels = "%b", #codes from abrivs https://rdrr.io/r/base/strptime.html
               expand = expansion()
  ...
  xlab("\nMonth") +
  ylab("Total Monthly SWE") +
  labs(title = "Total Monthly SWE above Pagosa Springs, CO",
       subtitle = "NCRS SNOTEL Guage 840 (2009 - 2019)\n") +
  theme(plot.title = element_text(vjust = 0.5)) +
  theme_ar()
