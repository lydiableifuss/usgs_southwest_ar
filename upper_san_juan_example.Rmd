...
huc8<- read_sf(dsn = here::here("HUC8_US.shp"),
                 layer = "HUC8_US") %>% 
...
USJ_HUC8<- read_sf(dsn = here::here("HUC8_US.shp"),
                 layer = "HUC8_US") %>% 
              st_transform(crs = 4326) %>% 
...
# nm_shp <- us_states(resolution = "high", states = "NM") %>%  
#   st_transform(crs = 4326)
# 
# co_nm <- rbind(co_shp, nm_shp)
...
sjp_map <- leaflet(data=sjp_dis_join) %>% 
