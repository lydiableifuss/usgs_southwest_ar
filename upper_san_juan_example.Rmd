...
kable_snotel <- kable(head(meta_snotel_df,20)) %>% 
 kable_styling(bootstrap_options = c("striped", 
                                     "bordered",
                                      "hover",
                                      "condensed",
...
sjp_snotel_2020 <- sjp_snotel_clean %>%
  filter(year > 2019)
...
sjp_swe_seasonal <- ggplot() +
  ...
  geom_line(data = sjp_snotel_2020, 
            ...
            aes(x = month, 
                y = total_month_swe))+
annotate(
    geom = "curve", x = 8, y = 900, xend = 4.2, yend = 670,
    curvature = .3, arrow = arrow(length = unit(3, "mm"))
  ) +
  annotate(geom = "text", x = 8, y = 900, label = "2020", hjust = "left") +
  theme_ar()+
  labs(title = "Total Monthly Snow-Water-Equivalent, Pagosa Springs, CO",
        subtitle = "NCRS SNOTEL Guage 840 (2009 - 2020)\n")+
  xlab("\nMonth") +
  ylab("Total Monthly SWE (in)\n") +
