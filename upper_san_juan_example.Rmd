...
kable_sj <- kable(head(kable_usj_sites, 10)) %>% 
  ...
  pack_rows("Upper San Juan", 1, 5, label_row_css = "background-color: #666; color: #fff;") %>% 
...
sjp_snotel <- grabNRCS.data(network = "SNTL", 
                            ...
                            DayBgn = "2009-10-01", 
                            DayEnd = "2019-09-30") %>% 
  select(Date, Snow.Water.Equivalent..in..Start.of.Month.Values) %>% 
  rename(Snow_Water_Equivalent = Snow.Water.Equivalent..in..Start.of.Month.Values)
...
sjp_swe_graph <- ggplot(data = sjp_snotel,
                        aes(Date, 
